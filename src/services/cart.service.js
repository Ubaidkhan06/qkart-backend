...
const addProductToCart = async (user, productId, quantity) => {
  ...
  if (!product) {
    throw new ApiError(
      ...
      "Product does not exist's in database"
    );
  }
  if (!cart) {
    try {
      const newCart = await Cart.create({ email: user.email });
      newCart.cartItems.push({ product: { ...product }, quantity: quantity });
      await newCart.save();
      return newCart;
      // console.log(newCart);
    } catch (err) {
    ...
    if (
      cart.cartItems.filter((item) => item.product._id == productId)
        .length !== 0
    ) {
      throw new ApiError(
        httpStatus.BAD_REQUEST,
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  // checking if product exist in collection
  if (!product) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "Product dosen't exist in database"
    );
  }

  // check if product in users cart
  // console.log(
  //   cart.cartItems.find((item) => item.product._id == productId).quantity
  // );
